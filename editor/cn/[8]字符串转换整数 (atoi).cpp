...
public:
    int myAtoi(string s) {
        int res=0,length=s.length(),i=0,p=1,broad=pow(2,31)-1;
        while(s[i]==' ') i++;
        if (s[i]=='-') {broad=0-pow(2,31);p=-1;i++;}
        else if (s[i]=='+') i++;
        ...
        if(s[i]>'0'&&s[i]<='9'){
            ...
            if(m<9) {
                while(m>=0) {res=res+(t[m]-'0')*pow(10,n-m);m--;}
                return res*p;
            }
            ...
            if(m==9){
                int delta;
                while(m>0) {res=res+(t[m]-'0')*pow(10,n-m);m--;}
                if(p==1)    delta=pow(2,31)-1-res;
                else    delta=pow(2,31)-res;

                if(delta>=(t[0]-'0')*pow(10,n)) {res=p*(res+(t[0]-'0')*pow(10,n));return res;}
                else return broad;
            }
